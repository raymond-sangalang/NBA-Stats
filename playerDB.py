...
class nbaDatabase:
    ...
    def connect_SQL(self):
        ...
        try:
            return sqlite3.connect(_DATABASE,
                                   detect_types=sqlite3.PARSE_DECLTYPES)  # return successful connection and acquire registered
    ...
    def add_to_tables(self, _conn, playerYear, player_stats, player_reb, playerObj, keyf):
        ...
        if self.check_records(_cur, playerObj[0]) is None:
    ...
    def getCurrentYearData(self, _conn):
        ...
        fetched = _cur.fetchone()[0]

        if fetched is None:
            return -1
        return fetched.astype(int)
    ...
    def update_Player(self, _cur, tup_vals=()):
        ...
        while key != -1:
            key = key.addUniq(f"{tup_vals[0]} {tup_vals[1]}")
